...
export class NewPasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.localStorageService.isTokenExpired()) {
      ...
      this.httpRequestService.checkTokenLogisticsCompanyResetPassword(this.token).subscribe({
        next: data => {
  ...
  submit() {
    ...
    const logisticsCompanyPassword: any = {
      password: null,
      token: null
    }

    logisticsCompanyPassword.password = this.password1.value;
    logisticsCompanyPassword.token = this.token;

    this.submitForm = false;

    this.httpRequestService.newPassword(logisticsCompanyPassword).subscribe({
      next: () => {
        this.responseServer = false;
