...
export class RegistrationComponent implements OnInit {
  ...
  passportPassport: any;
  passportMedical: any;

// Изображение выбрано и оно корректно true
  imagePassportOk = false;
  imageMedicalOk = false;
  ...
  get p_img_medical_checkup() {
    return this.loginForm.controls['p_img_medical_checkup'];
  }
  ...
  submit() {
    ...
    formData.append('imagePassport', this.passportPassport)
    formData.append('imageMedical', this.passportMedical)
    formData.append('registrationLogisticsPerson', JSON.stringify(person))
    if (this.imagePassportOk && this.imageMedicalOk) {
      this.httpRequestService.registration(formData).subscribe({
        ...
        error:(err) => {
          if (err.error.status == 400
            && err.error.message == 'LogisticsPerson exist') {
  ...
  pickedImagePassport(imageInput: any) {

    if (imageInput.target.files[0]) {
      this.passportPassport = imageInput.target.files[0]

      if (this.passportPassport.type.toString() !== 'image/jpeg' && this.passportPassport.type.toString() !== 'image/png') {
        this.p_img_passport.setErrors({'imageErrorNoCorrect': true});
        this.imagePassportOk = false;
        this.passportPassport = null;

      }

      else if (this.passportPassport.size > 10485760) {
        this.p_img_passport.setErrors({'imageErrorMB': true});
        this.imagePassportOk = false;
        this.passportPassport = null;
      }

      else if (this.passportPassport.size < 20480) {
  ...
  pickedImageMedical(imageInput: any) {

    if (imageInput.target.files[0]) {
      this.passportMedical = imageInput.target.files[0]

      if (this.passportMedical.type.toString() !== 'image/jpeg' && this.passportMedical.type.toString() !== 'image/png') {
        this.p_img_medical_checkup.setErrors({'imageErrorNoCorrect': true});
        this.imageMedicalOk = false;
        this.passportMedical = null;

      }

      else if (this.passportMedical.size > 10485760) {
        this.p_img_medical_checkup.setErrors({'imageErrorMB': true});
        this.imageMedicalOk = false;
        this.passportMedical = null;
      }

      else if (this.passportMedical.size < 20480) {
        this.p_img_medical_checkup.setErrors({'imageErrorByte': true});
        this.imageMedicalOk = false;
        this.passportMedical = null;
      }

      else {
        this.imageMedicalOk = true;
      }
