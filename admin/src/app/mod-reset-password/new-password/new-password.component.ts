...
export class NewPasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.localStorageService.isTokenExpired()) {
      ...
      this.httpRequestService.checkTokenAdminResetPassword(this.token).subscribe({
        next: data => {
  ...
  submit() {
    ...
    adminPassword.password = this.password1.value;
    adminPassword.token = this.token;

    this.submitForm = false;

    this.httpRequestService.newPassword(adminPassword).subscribe({
      next: () => {
        this.responseServer = false;
