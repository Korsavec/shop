...
@Component({
  selector: 'app-new-password-seller-person',
  templateUrl: './new-password-seller-person.component.html',
  styleUrls: ['./new-password-seller-person.component.css']
})
export class NewPasswordSellerPersonComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.storeUserService.isTokenExpired()) {
      ...
      this.httpClientService.checkServerTokenSellerPersonResetPassword(this.token).subscribe({
        next: data => {
  ...
  submit() {
    ...
    this.httpClientService.newPasswordSellerPerson(userPassword).subscribe({
      next: () => {
        this.responseServer = false;
