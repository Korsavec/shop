...
@Component({
  selector: 'app-new-password-user',
  templateUrl: './new-password-user.component.html',
  styleUrls: ['./new-password-user.component.css']
})
export class NewPasswordUserComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.storeUserService.isTokenExpired()) {
      ...
      this.httpClientService.checkServerTokenUserResetPassword(this.token).subscribe({
        next: data => {
  ...
  submit() {
    ...
    this.httpClientService.newPasswordUser(userPassword).subscribe({
      next: () => {
        this.responseServer = false;
