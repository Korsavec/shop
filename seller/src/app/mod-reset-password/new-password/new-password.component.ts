...
export class NewPasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.localStorageService.isTokenExpired()) {
      ...
      this.httpRequestService.checkTokenSellerResetPassword(this.token).subscribe({
        next: data => {
  ...
  submit() {
    ...
    const sellerPassword: any = {
      password: null,
      token: null
    }

    sellerPassword.password = this.password1.value;
    sellerPassword.token = this.token;

    this.submitForm = false;

    this.httpRequestService.newPassword(sellerPassword).subscribe({
      next: () => {
        this.responseServer = false;
